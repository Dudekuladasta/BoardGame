...
pipeline {
  ...
  stages {
      ...
      steps {
        script {
          withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
            sh "docker build -t dastaaws/boardshack:latest ."
          }
        }
      }
    }
    ...
    stage('Docker Image Scan') {
      steps {
        sh "trivy image --format table -o trivy-image-report.html dastaaws/boardshack: latest "
      }
    }
    ...
    stage('Push Docker Image') {
      steps {
        script {
          withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
            sh "docker push dastaaws/boardshack:latest"
          }
        }
      }
    }
